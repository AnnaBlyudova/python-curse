...
if not TOKEN:
    pytest.skip("Нет токена. Установи переменную окружения YOUGILE_TOKEN",
...
def test_create_project_negative_empty_title():
    ...
    response = requests.post(base_url + '/projects', headers=headers,
                             json={"title": ""})
...
def test_update_project_negative_invalid_id():
    ...
    response = requests.put(f"{base_url}/projects/{fake_id}",
                            headers=headers, json={"title": "Новое"})
